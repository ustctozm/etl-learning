...
class CustomKafkaWriter<IN>
    ...
    private final CustomKafkaRecordSerializationSchema<IN> recordSerializer;
    private final CustomKafkaRecordSerializationSchema.KafkaSinkContext kafkaSinkContext;
    ...
    CustomKafkaWriter(
            ...
            CustomKafkaRecordSerializationSchema<IN> recordSerializer,
        ...
        this.kafkaSinkContext =
                new CustomDefaultKafkaSinkContext(
        ...
        public void onCompletion(RecordMetadata metadata, Exception exception) {
            ...
            if (metadata == null) {
                ...
                if (asyncProducerException == null) {
                    asyncProducerException = decorateException(null, exception, producer);
                }
            ...
            if (exception != null) {
                ...
                try {
                    CustomKafkaRecordSerializationSchemaBuilder.CustomKafkaRecordSerializationSchemaWrapper<IN> wrapper = null;
                    if (recordSerializer != null && metadata != null) {
                        LOG.warn("recordSerializer start getTopicPartitions, {} and partiton= {}", metadata.topic(), metadata.partition());
                        wrapper = (CustomKafkaRecordSerializationSchemaBuilder.CustomKafkaRecordSerializationSchemaWrapper<IN>) recordSerializer;

                        TopicPartitions topicPartitions = wrapper.getTopicPartitions();
                        ...
                        if (newPartitions == null || newPartitions.length == 0 || (topicPartitions.getTopicPartitions(topic) != null &&
                                newPartitions.length <= topicPartitions.getTopicPartitions(topic).length * 0.3)) {
                            mailboxExecutor.submit(
                                    () -> {
                                        // Checking for exceptions from previous writes
                                        checkAsyncException();
                        ...
                        int partition = wrapper.getPartitioner().partition(
                                value,
                                record.key(),
                                record.value(),
                                topic,
                        ...
                        LOG.warn("retry send record, old target partition={} new partition={}, left {} times.",
                                badPartition,
                                partition,
                                retry);
                    }
