...
public class CustomKafkaSinkBuilder<IN> {
    ...
    private CustomKafkaRecordSerializationSchema<IN> recordSerializer;
     ...
     */
    public CustomKafkaSinkBuilder<IN> setDeliveryGuarantee(DeliveryGuarantee deliveryGuarantee) {
     ...
     * @return {@link CustomKafkaSinkBuilder}
     */
    public CustomKafkaSinkBuilder<IN> setKafkaProducerConfig(Properties props) {
        checkNotNull(props);
    ...
    public CustomKafkaSinkBuilder<IN> setProperty(String key, String value) {
        checkNotNull(key);
        Arrays.stream(warnKeys)
                .filter(key::equals)
                .forEach(k -> LOG.warn("Overwriting the '{}' is not recommended", k));
