...
/*
 *  All code from following url. we just add custom code for enabling that failed message resend by our custom callback.
 *  https://github.com/apache/flink/blob/release-1.17.1/flink-connectors/flink-connector-kafka/src/main/java/org/apache/flink/streaming/connectors/kafka/FlinkKafkaProducer.java
 */

package net.qihoo.ads.patched.org.apache.flink.streaming.connectors.kafka;

import net.qihoo.ads.flink.kafka.BalancedFlinkKafkaPartitioner;
import net.qihoo.ads.flink.kafka.RetryableProducerCallback;
import net.qihoo.ads.flink.kafka.TopicPartitions;
...
public class FlinkKafkaProducer<IN>
    ...
    /** Default partitioner for try to send record. */
    private FlinkKafkaPartitioner<IN> defaultFlinkKafkaPartitioner;
    ...
    /** Custom class for replacing topicPartitionsMap . */
    public TopicPartitions topicPartitions;
    /** If a message send failed, we will try actualCallbackRetryTimes times to send this message in callback. */
    public static final String RETRY_TIMES_CUSTOM_CALLBACK = "callback.retry.times";
    public static final String defaultCallbackRetryTimes = "3";
    public int actualCallbackRetryTimes;
    /** Ensure checkpoint point aligns with all kafka message which partial send in callback. */
    public static final String RETRY_TIMES_CHECKPOINT_FLUSH = "checkpoint.flush.retry.times";
    public static final String defaultCheckpointFlushRetryTimes = "150";
    public int actualCheckpointFlushRetryTimes;
    /** Clean bad partitions, attempt to recover all partitions. */
    public static final String CLEAN_BAD_PARTITIONS_CHECKPOINT_INTERVAL = "bad.partitions.clean.cp.times";
    public static final String defaultCleanBadPartitionsCpInterval = "6";
    public int actualCleanBadPartitionsCpInterval;
    ...
    public void open(Configuration configuration) throws Exception {
        ...
        // Get param from GlobalJobParameters which could set in MainClass.
        ...
        actualCallbackRetryTimes =
                Integer.parseInt(env.getConfig().getGlobalJobParameters().toMap().
                        getOrDefault(RETRY_TIMES_CUSTOM_CALLBACK, defaultCallbackRetryTimes)
                );
        actualCheckpointFlushRetryTimes =
                Integer.parseInt(env.getConfig().getGlobalJobParameters().toMap().
                        getOrDefault(RETRY_TIMES_CHECKPOINT_FLUSH, defaultCheckpointFlushRetryTimes)
                );
        actualCleanBadPartitionsCpInterval =
                Integer.parseInt(env.getConfig().getGlobalJobParameters().toMap().
                        getOrDefault(CLEAN_BAD_PARTITIONS_CHECKPOINT_INTERVAL, defaultCleanBadPartitionsCpInterval)
                );
        topicPartitions = new TopicPartitions(actualCleanBadPartitionsCpInterval);
        retryMeter = ctx.getMetricGroup().meter("retry_callback", new DropwizardMeterWrapper(new com.codahale.metrics.Meter()));
        if (flinkKafkaPartitioner != null) {
            flinkKafkaPartitioner.open(
                    ctx.getIndexOfThisSubtask(), ctx.getNumberOfParallelSubtasks());
        }
        defaultFlinkKafkaPartitioner = new BalancedFlinkKafkaPartitioner<>();
        defaultFlinkKafkaPartitioner.open(getRuntimeContext().getIndexOfThisSubtask(), getRuntimeContext().getNumberOfParallelSubtasks());
    ...
    public void invoke(
        ...
        if (keyedSchema != null) {
            ...
            int[] partitions = topicPartitions.getAvailablePartitions(targetTopic);
            if (null == partitions) {
                partitions = getPartitionsByTopic(targetTopic, transaction.producer);
                topicPartitions.addPartitionsForTopic(targetTopic, partitions);
            } else if (partitions.length == 0) {
                throw new RuntimeException("We have no healthy partitions for topic=" + targetTopic + "!");
            }
            if (flinkKafkaPartitioner != null) {
                record =
                        new ProducerRecord<>(
                ...
                if (!topicPartitions.isHealthyForTopic(targetTopic)) {
                    int partition = defaultFlinkKafkaPartitioner.partition(
                            next,
                            serializedKey,
                            serializedValue,
                            targetTopic,
                    ...
                    record =
                            new ProducerRecord<>(
                                    targetTopic, partition, timestamp, serializedKey, serializedValue);
                }
                else {
                    record =
                            new ProducerRecord<>(
            ...
            if (kafkaSchema instanceof KafkaContextAware) {
                ...
                if ((record.partition() == null && !topicPartitions.isHealthyForTopic(targetTopic))
                        || (record.partition() != null && topicPartitions.isBadPartition(record.topic(), record.partition()))) {
                    int partition = defaultFlinkKafkaPartitioner.partition(
                            next,
                            record.key(),
                            record.value(),
        ...
        FlinkKafkaInternalProducer<byte[], byte[]> producer = transaction.getProducer();
        switch (semantic) {
            // EXACTLY_ONCE cannot retry send failed message by using current producer instance, Once failed this producer is unuseful.
            case EXACTLY_ONCE:
                producer.send(record, callback);
                break;
            case AT_LEAST_ONCE:
            case NONE:
                RetryableProducerCallback<IN> producerCallback = new RetryableProducerCallback<>(
                        next,
                        producer,
                        actualCallbackRetryTimes,
                        flinkKafkaPartitioner,
    ...
    public void close() throws FlinkKafkaException {
        ...
        try {
            ...
            try {
                ...
                LOG.error("Error release producer in TwoPhaseCommitSinkFunction.", e);
            }
     ...
     * If it ensures that CustomProducerCallback && flush should be finished completely, We should make sure that
     * checkpoint.timeout >= CustomProducerCallback timeout(default 120s) + flush timeout(default 60s).
     * @param transaction
     */
    private void flush(FlinkKafkaProducer.KafkaTransactionState transaction)
        ...
        if (transaction.producer != null) {
            ...
            switch (semantic) {
                case AT_LEAST_ONCE:
                case NONE:
                    // retry flushing record which send by producer in custom callback to broker.
                    // flush() method using Snapshot to get records which in RecordAccumulator.
                    // When first call flush() method below, we just get three records.
                    // flush() will do 1.send these three records 2. waiting callback finished, we retry send record2.
                    // When second call flush() method. we get record2.
                    // similarly this flush() will do 1.send record2 2. waiting callback of record2 succeed.
                    //-----------------------/——custom callback——>[ record2]------|flush()|--
                    //----------------------/-------------------------------------------------
                    //---------------------/--------------------------------------------------
                    //----[record1, record2, record3]---------------|flush()｜----------------
                    //------------------------------------------------------------------------
                    //_______________________________timestamp increase________________________
                    int retryTimes = actualCheckpointFlushRetryTimes;
                    try {
                        while (pendingRecords.get() > 0 && retryTimes-- > 0) {
                            Thread.sleep(1000);
                            transaction.producer.flush();
                            LOG.info("Waiting pendingRecords be flushed to zero, sleep one second.");
                        }
                    } catch (InterruptedException exception) {
                        throw new FlinkKafkaException(FlinkKafkaErrorCode.EXTERNAL_ERROR, "pendingRecords");
                    }
                    break;
            }
        }
    ...
    public void snapshotState(FunctionSnapshotContext context) throws Exception {
        ...
        // We get cpId here,It may cause that a batch callback contain two cpId (a, a+1)
        // But this can be accepted, It actually should be set in prepareSnapshotPreBarrier() or notifyCheckpointComplete()
        topicPartitions.setCheckPointId(context.getCheckpointId());
        ...
        // clean topicBadPartitionsMap when cp times achieve actualCleanBadPartitionsCpInterval.
        topicPartitions.cleanBadPartition();
        String badPartitionInfo = topicPartitions.getBadPartitionInfo();
        if (badPartitionInfo != null) {
            LOG.warn("Now existing following bad partitions: " + badPartitionInfo);
        }
    }
